...
const contributionCells = Array.from({ length: 84 }, (_, index) => {
  const levels = ["bg-slate-800", "bg-cyan-950", "bg-cyan-800", "bg-cyan-500", "bg-emerald-500"];
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-100">Pinned repositories</h3>
                <p className="font-mono text-xs text-slate-500">git status --portfolio</p>
              </div>
            </div>
            <div className="mt-6 grid grid-cols-12 gap-1" aria-label="Contribution activity mockup">
            ...
            <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-950/80 p-4 font-mono text-sm text-slate-300">
              <p><span className="text-cyan-300">$</span> pnpm build</p>
              <p className="mt-2 text-emerald-400">✓ production bundle ready</p>
              <p className="mt-1 text-slate-500">reverse proxy: nginx | caddy</p>
            </div>
          </motion.div>
            ...
            {repositories.map((repo) => (
              ...
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <h3 className="font-mono text-base font-semibold text-cyan-300">{repo.name}</h3>
                    <p className="mt-2 text-sm leading-6 text-slate-400">{repo.description}</p>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {repo.stack.map((tech) => (
                    <span key={tech} className="rounded-full border border-slate-700 bg-slate-950/70 px-3 py-1 font-mono text-xs text-slate-300">
                      {tech}
                    </span>
                  ))}
                </div>
